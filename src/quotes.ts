export const quotes = [
  {
    chapter: 2,
    chapter_name: "Sankhya Yoga",
    sanskrit: "न जायते म्रियते वा कदाचिन्नायं भूत्वा भविता वा न भूयः ।",
    english: "For the soul there is neither birth nor death at any time.",
  },
  {
    chapter: 2,
    chapter_name: "Sankhya Yoga",
    sanskrit: "वासांसि जीर्णानि यथा विहाय नवानि गृह्णाति नरोऽपराणि ।",
    english:
      "As a person puts on new garments, giving up old ones, the soul accepts new bodies.",
  },
  {
    chapter: 2,
    chapter_name: "Sankhya Yoga",
    sanskrit: "कर्मण्येवाधिकारस्ते मा फलेषु कदाचन ।",
    english:
      "You have a right to perform your duty, but not to the fruits of action.",
  },
  {
    chapter: 2,
    chapter_name: "Sankhya Yoga",
    sanskrit: "समत्वं योग उच्यते ।",
    english: "Equanimity of mind is called Yoga.",
  },
  {
    chapter: 3,
    chapter_name: "Karma Yoga",
    sanskrit: "नियतं कुरु कर्म त्वं कर्म ज्यायो ह्यकर्मणः ।",
    english:
      "Perform your prescribed duty, for action is better than inaction.",
  },
  {
    chapter: 3,
    chapter_name: "Karma Yoga",
    sanskrit: "यज्ञार्थात्कर्मणोऽन्यत्र लोकोऽयं कर्मबन्धनः ।",
    english:
      "Work done as a sacrifice for Vishnu has to be performed, otherwise work binds one to this world.",
  },
  {
    chapter: 4,
    chapter_name: "Jnana Karma Sanyas Yoga",
    sanskrit: "यदा यदा हि धर्मस्य ग्लानिर्भवति भारत ।",
    english:
      "Whenever there is decline in righteousness and rise of unrighteousness, O Arjuna, I manifest Myself.",
  },
  {
    chapter: 4,
    chapter_name: "Jnana Karma Sanyas Yoga",
    sanskrit: "परित्राणाय साधूनां विनाशाय च दुष्कृताम् ।",
    english:
      "To protect the righteous, to annihilate the wicked, and to reestablish dharma, I appear millennium after millennium.",
  },
  {
    chapter: 4,
    chapter_name: "Jnana Karma Sanyas Yoga",
    sanskrit: "कर्मणैव हि संसिद्धिमास्थिता जनकादयः ।",
    english: "Indeed, by action alone did Janaka and others attain perfection.",
  },
  {
    chapter: 5,
    chapter_name: "Karma Sanyas Yoga",
    sanskrit: "योगयुक्तो विशुद्धात्मा विजितात्मा जितेन्द्रियः ।",
    english:
      "One who is disciplined in yoga, pure in mind, and has conquered the self, is liberated.",
  },
  {
    chapter: 6,
    chapter_name: "Dhyana Yoga",
    sanskrit: "उद्धरेदात्मनाऽऽत्मानं नात्मानमवसादयेत् ।",
    english:
      "Elevate yourself through the power of your mind, and do not degrade yourself.",
  },
  {
    chapter: 6,
    chapter_name: "Dhyana Yoga",
    sanskrit: "योगिनामपि सर्वेषां मद्गतेनान्तरात्मना ।",
    english:
      "Of all yogis, one who always abides in Me with great faith is most intimately united with Me.",
  },
  {
    chapter: 7,
    chapter_name: "Jnana Vijnana Yoga",
    sanskrit: "मया ततमिदं सर्वं जगदव्यक्तमूर्तिना ।",
    english:
      "By Me, in My unmanifested form, this entire universe is pervaded.",
  },
  {
    chapter: 7,
    chapter_name: "Jnana Vijnana Yoga",
    sanskrit: "बहूनां जन्मनामन्ते ज्ञानवान्मां प्रपद्यते ।",
    english:
      "After many births and deaths, one who is truly wise surrenders unto Me.",
  },
  {
    chapter: 8,
    chapter_name: "Akshara Brahma Yoga",
    sanskrit: "अन्तकाले च मामेव स्मरन्मुक्त्वा कलेवरम् ।",
    english: "Whoever remembers Me at the time of death attains My being.",
  },
  {
    chapter: 9,
    chapter_name: "Raja Vidya Raja Guhya Yoga",
    sanskrit: "पत्रं पुष्पं फलं तोयं यो मे भक्त्या प्रयच्छति ।",
    english:
      "If one offers Me with love a leaf, a flower, a fruit, or water, I accept it.",
  },
  {
    chapter: 9,
    chapter_name: "Raja Vidya Raja Guhya Yoga",
    sanskrit: "अनन्याश्चिन्तयन्तो मां ये जनाः पर्युपासते ।",
    english:
      "Those who always worship Me with exclusive devotion, I take care of what they lack and preserve what they have.",
  },
  {
    chapter: 9,
    chapter_name: "Raja Vidya Raja Guhya Yoga",
    sanskrit: "सर्वधर्मान्परित्यज्य मामेकं शरणं व्रज ।",
    english: "Abandon all varieties of dharma and surrender unto Me alone.",
  },
  {
    chapter: 10,
    chapter_name: "Vibhuti Yoga",
    sanskrit: "अहं सर्वस्य प्रभवो मत्तः सर्वं प्रवर्तते ।",
    english: "I am the source of all spiritual and material worlds.",
  },
  {
    chapter: 10,
    chapter_name: "Vibhuti Yoga",
    sanskrit: "यद्वा भूतिगुणयुक्तं स्वप्नं वा प्रभवो यशः ।",
    english:
      "Know that all beautiful, glorious, and mighty creations spring from but a spark of My splendor.",
  },
  {
    chapter: 11,
    chapter_name: "Vishwarupa Darshana Yoga",
    sanskrit: "दिवि सूर्यसहस्रस्य भवेद्युगपदुत्थिता ।",
    english:
      "If a thousand suns were to rise in the sky at once, their radiance might resemble the splendor of that form.",
  },
  {
    chapter: 12,
    chapter_name: "Bhakti Yoga",
    sanskrit: "मय्यावेश्य मनो ये मां नित्ययुक्ता उपासते ।",
    english:
      "Those who fix their minds on Me and worship Me with unwavering devotion, I consider them perfect yogis.",
  },
  {
    chapter: 13,
    chapter_name: "Kshetra Kshetragya Vibhaaga Yoga",
    sanskrit: "क्षेत्रज्ञं चापि मां विद्धि सर्वक्षेत्रेषु भारत ।",
    english: "Know that I am also the knower in all bodies, O Arjuna.",
  },
  {
    chapter: 15,
    chapter_name: "Purushottama Yoga",
    sanskrit: "ममैवांशो जीवलोके जीवभूतः सनातनः ।",
    english:
      "The living entities in this conditioned world are My eternal fragmental parts.",
  },
  {
    chapter: 18,
    chapter_name: "Moksha Sanyas Yoga",
    sanskrit: "मन्मना भव मद्भक्तो मद्याजी मां नमस्कुरु ।",
    english:
      "Always think of Me, become My devotee, worship Me, and offer your homage unto Me.",
  },
];
